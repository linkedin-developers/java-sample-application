...
public class LMSController {
	...
	public String postBody(@RequestBody String POST_ARRAY, final Model model) {
		...
		switch(POST_ARRAY){
			case "Check_token=Check+Token": response = lmsTemplate.getForObject(SERVER_URL  + "refresh_token", String.class);
			  break;
			case "Find_ad_account_by_authenticated_user=Find+Ad+Accounts": response = lmsTemplate.getForObject(SERVER_URL  + "Find_ad_account_by_authenticated_user", String.class);
				break;
			case "Get_user_org_access=Find+Ad+Account+roles" : response = lmsTemplate.getForObject(SERVER_URL  + "Get_user_org_access", String.class);
				break;
			case "Fetch_ad_account=Fetch+Ad+Account+by+ID": response = lmsTemplate.getForObject(SERVER_URL  + "Fetch_ad_accoun", String.class);
				break;
			case "Get_ad_account_user=Get+Ad+Account+User": response = lmsTemplate.getForObject(SERVER_URL  + "Get_ad_account_user", String.class);
				break;
			case "Find_ad_account_user_by_account=Find+Ad+Account+Users": response = lmsTemplate.getForObject(SERVER_URL  + "Find_ad_account_user_by_account", String.class);
				break;
